...
def evaluate_model(network, data, data_object):
    ...
    for batch in tqdm(data):
        ...
        loss = accuracy_factor * loss_fn(output, y) / ((torch.subtract(y, predicted) != 0).sum()) + class_factor * (torch.square(torch.subtract(y, predicted)).sum())
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        loss = accuracy_factor * loss_fn(output, y) / ((torch.subtract(y, predictted_output) != 0).sum()) + class_factor * (torch.square(torch.subtract(y, predictted_output)).sum())
