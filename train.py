...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        # if epoch < 15:    
        loss = loss_fn(output, y) + torch.exp(class_factor * ((torch.subtract(y, predictted_output) != 0).sum())) + class_factor * torch.absolute(torch.sum(y) - torch.sum(predictted_output))
