...
loss_fn = nn.NLLLoss(weight=torch.tensor([1.0, 2.735015773,2.842622951,11.40789474,13.76190476,14.45]).to(device))
# optimizer = torch.optim.Adam(network.parameters(), weight_decay = 1e-3, lr=0.01)
optimizer = torch.optim.Adam(network.parameters(), lr =0.01)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience = 0, factor = 0.5, verbose = True)
n_epochs = 60
