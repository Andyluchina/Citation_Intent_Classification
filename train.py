...
def evaluate_model(network, data, data_object):
    ...
    for batch in tqdm(data):
        ...
        loss = accuracy_factor * torch.log(torch.square(torch.subtract(y, predicted)).sum()) * loss_fn(output, y) + class_factor * (torch.square(torch.subtract(y, predicted)).sum())
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        loss = accuracy_factor * torch.log(torch.square(torch.subtract(y, predictted_output)).sum()) * loss_fn(output, y) + class_factor * (torch.square(torch.subtract(y, predictted_output)).sum())
