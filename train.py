...
optimizer = torch.optim.Adam(network.parameters(), weight_decay = 2e-4, lr=0.001)
# optimizer = torch.optim.Adam(network.parameters(), lr=0.01)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience = 2, factor = 0.2, verbose = True)
n_epochs = 60
class_factor = 0.4
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        if epoch < 15:    
            loss = loss_fn(output, y) + class_factor * torch.sum(torch.absolute(torch.subtract(y, predictted_output)))
        else:
