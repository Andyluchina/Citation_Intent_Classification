...
def evaluate_model(network, data, data_object):
    ...
    for batch in tqdm(data):
        ...
        loss = accuracy_factor * loss_fn(output, y) + class_factor * (torch.subtract(y, predicted)!=0).sum()
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        loss = accuracy_factor * loss_fn(output, y) + class_factor * (torch.subtract(y, predictted_output)!=0).sum()
