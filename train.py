...
optimizer = torch.optim.Adam(network.parameters(), weight_decay = 1e-7, lr=0.00003)
...
def evaluate_model(network, data, data_object):
    ...
    for batch in tqdm(data):
        ...
        loss = loss_fn(output, y) + class_factor * ((torch.subtract(y, predicted) != 0).sum()) + sum_factor * torch.absolute(torch.sum(y) - torch.sum(predictted_output))
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        # if epoch < 15:    
        loss = loss_fn(output, y) + class_factor * ((torch.subtract(y, predictted_output) != 0).sum()) + sum_factor * torch.absolute(torch.sum(y) - torch.sum(predictted_output))
