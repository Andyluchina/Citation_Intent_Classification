...
def evaluate_model(network, data, data_object):
    ...
    for batch in tqdm(data):
        ...
        loss = torch.log(torch.square(torch.subtract(y, predicted)).sum()) * loss_fn(output, y) + class_factor * (torch.square(torch.subtract(y, predicted)).sum())
        print("Accuracy Loss: ", torch.log(torch.square(torch.subtract(y, predicted)).sum()) * loss_fn(output, y))
...
for epoch in range(n_epochs):
    ...
    for batch in tqdm(train_loader):
        ...
        loss = torch.log(torch.square(torch.subtract(y, predictted_output)).sum()) * loss_fn(output, y) + class_factor * (torch.square(torch.subtract(y, predictted_output)).sum())
